...
def get_other_inflation(df_cpi, year):
	df_c = df_cpi
	other_inflation = df_c.loc[year,'cpi']/df_c.loc[2017,'cpi']
...
def get_hist_cost_index(year, df_hist_cost):
 hist_cost_index = df_hist_cost.loc[year,'cost_index']/df_hist_cost.loc[2021,'cost_index']
...
project_size_list = [0.1,0.2,0.5,1,2] #testing
...
def final_table(project_size, poi, state, num_modules):
	"""
	...
	sales_tax = df_state.loc[state, ('Sales Tax', 'Commercial')]
	...
	df_final.loc['Inverter Only',state] = ft.get_inverter_cost(state, nec_2017_list, df_inputs)
	...
	df_final.loc['Permitting & Interconnection', state], permitting_cost = ft.get_permitting_cost(state, df_state, df_inputs, project_size, poi, df_wage, df_labor_weight, df_utility, other_inflation)
...
for project_size in project_size_list:
	...
	dc_ac_ratio = df_inputs.loc['ILR', A]
	poi = round(project_size/dc_ac_ratio,2)

	for state in state_list:

		df_final = final_table(num_modules*watts, poi, state, num_modules)
	...
	df_state_results[str(project_size)+" MWdc"+" "+str(poi)+" MWac"] = df_all_states
