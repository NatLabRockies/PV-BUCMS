...
def final_table(project_size, state, df_inv_calc, num_modules):
	...
	df_final.loc['Structural BOS',state], df_structural_bos, df_bom, size_factor = ft.get_structural_bos(df_utility,project_size, num_modules, df_inputs, df_state, state, inflation, df_module_db, year, df_torque_tube, df_pipe, df_rail_clamp, steel_inflation, df_inv_calc)
	...
	df_final.loc['Epc Overhead',state], df_total_installed_cost, df_bare_cost, trans_dist_list = ft.get_oh_p_cost(df_inputs, df_install, df_utility, df_state, state, inflation, project_size, df_labor_table, df_labor_weight, df_wage, df_epc)
...
export_path = path + "//results//Utility Benchmark Table for Chart.xlsx"
