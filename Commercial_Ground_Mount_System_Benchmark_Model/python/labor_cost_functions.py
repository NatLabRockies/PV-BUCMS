...
def preconstruction_surveys(df_utility, df_inputs, df_state, num_modules, project_size, state, hist_cost_index):
    ...
    # print(average,['Job Quantity'],df_pre_sur['Labor Hours'],df_pre_sur['Material Cost Per Unit'],df_pre_sur['Equipment Cost Per Unit'])
    return df_pre_sur

def access_roads_and_parking(df_utility, df_state, project_size, state, hist_cost_index, df_pre_sur):
    ...
    df_acc_roa['Material Cost Per Unit'] = material_bare_cost('Access Roads and Parking', df_state, df_utility, state)*hist_cost_index
    df_acc_roa['Equipment Cost Per Unit'] = equipment_bare_cost('Access Roads and Parking', df_state, df_utility, state)*hist_cost_index
    
    return df_acc_roa

def security_fencing(df_utility, df_pre_sur, df_state, hist_cost_index, state):
    ...
    df_sec_fen['Material Cost Per Unit'] = material_bare_cost('Security Fencing', df_state, df_utility, state)*hist_cost_index
    df_sec_fen['Equipment Cost Per Unit'] = equipment_bare_cost('Security Fencing', df_state, df_utility, state)*hist_cost_index
    return df_sec_fen

def temp_office(df_utility, df_state, hist_cost_index, state, project_size):
    ...
    if project_size <= 0.1:
        df_tem_off['Material Cost Per Unit'] = material_bare_cost('Temporary Office', df_state, df_utility, state)*hist_cost_index*0.5
    elif project_size <= 0.2:
        df_tem_off['Material Cost Per Unit'] = material_bare_cost('Temporary Office', df_state, df_utility,state) * hist_cost_index * 0.8
    else:
        df_tem_off['Material Cost Per Unit'] = material_bare_cost('Temporary Office', df_state, df_utility,state) * hist_cost_index
    df_tem_off['Equipment Cost Per Unit'] = equipment_bare_cost('Temporary Office', df_state, df_utility, state)*hist_cost_index
    return df_tem_off

def storage_box(df_utility, df_state, num_modules, hist_cost_index, state, project_size):
    ...
    df_sto_box['Material Cost Per Unit'] = material_bare_cost('Storage Box', df_state, df_utility, state)*hist_cost_index
    df_sto_box['Equipment Cost Per Unit'] = equipment_bare_cost('Storage Box', df_state, df_utility, state)*hist_cost_index
    return df_sto_box

def om_building(df_utility, df_state, hist_cost_index, state, project_size):
    df_om_build = pd.DataFrame(index=['O & M Building'])
    if project_size < 0.5:
        df_om_build['Job Quantity'] = float(df_utility.loc['O & M Building','Job Quantity'])* project_size/2
    else:
    ...
    df_om_build['Material Cost Per Unit'] = material_bare_cost('O & M Building', df_state, df_utility, state)*hist_cost_index
    df_om_build['Equipment Cost Per Unit'] = equipment_bare_cost('O & M Building', df_state, df_utility, state)*hist_cost_index
    return df_om_build

def site_prep(df_inputs, df_utility, df_state, df_inv_calc, hist_cost_index, state, df_pre_sur):
    ...
    if df_inputs.loc['Environment Saving?', A].lower()=='no':
        ...
        env = acre*avg*(1.0-df_inputs.loc['Environment Land Acquisition', C])
...
def foundation_inverter(df_utility, project_size, df_inputs, df_inv_calc, df_state, hist_cost_index, state):
    ...
    df_fou_inv['Material Cost Per Unit'] = material_bare_cost('Foundation for inverter/transformer/PVCS/Substation', df_state, df_utility, state)*hist_cost_index
    df_fou_inv['Equipment Cost Per Unit'] = equipment_bare_cost('Foundation for inverter/transformer/PVCS/Substation', df_state, df_utility, state)*hist_cost_index
    
    return df_fou_inv

def trenches(df_utility,project_size, num_modules, df_inputs, df_inv_calc, df_state, hist_cost_index, state):
    ...
    df_trenches['Material Cost Per Unit'] = material_bare_cost('Trenches', df_state, df_utility, state)*hist_cost_index
    df_trenches['Equipment Cost Per Unit'] = equipment_bare_cost('Trenches', df_state, df_utility, state)*hist_cost_index
    
    return df_trenches

def foundation_vertical_support(df_utility, df_inputs, df_bom, project_size, hist_cost_index, df_state, steel_inflation, df_pipe, state):
    ...
    df_found_vert_supp['Equipment Cost Per Unit'] = equipment_bare_cost('Foundation for Vertical Support', df_state, df_utility, state)*hist_cost_index
    
    return df_found_vert_supp, size_factor

def horizontal_support(df_utility, df_inputs, df_bom, hist_cost_index, df_state, steel_inflation, df_torque_tube, state, size_factor):
    ...
    df_hor_supp['Equipment Cost Per Unit'] = equipment_bare_cost('Horizontal Support Structures', df_state, df_utility, state)*hist_cost_index

    return df_hor_supp

def welding_or_bolting(df_utility, df_inputs, df_bom, hist_cost_index, df_state, state):
    ...
    df_weld_bolt['Material Cost Per Unit'] = material_bare_cost('Welding or Bolting', df_state, df_utility, state)*hist_cost_index
    df_weld_bolt['Equipment Cost Per Unit'] = equipment_bare_cost('Welding or Bolting', df_state, df_utility, state)*hist_cost_index
    
    return df_weld_bolt

def modules_mounting(df_utility, df_inputs, df_bom, hist_cost_index, df_state, num_modules, state, df_rail_clamp, steel_inflation, size_factor):
    ...
    df_mod_mount['Equipment Cost Per Unit'] = equipment_bare_cost('Modules Mounting', df_state, df_utility, state)*hist_cost_index
    
    return df_mod_mount

def t_connection(df_utility, df_bom, hist_cost_index, df_state, state, size_factor):
    ...
    df_t_conn['Equipment Cost Per Unit'] = equipment_bare_cost('T- Connection', df_state, df_utility, state)*hist_cost_index
    
    return df_t_conn

def ujoint_driveline(df_inputs,df_utility,df_state, num_modules, hist_cost_index, state, size_factor):
    ...
    df_ujoint['Material Cost Per Unit'] = material_bare_cost('U-Joint & Driveline', df_state, df_utility, state)*hist_cost_index*size_factor
    df_ujoint['Equipment Cost Per Unit'] = equipment_bare_cost('U-Joint & Driveline', df_state, df_utility, state)*hist_cost_index
    return df_ujoint

def slave_gearbox(df_utility, df_inputs,hist_cost_index, df_state, state, num_modules, size_factor):
    ...
    df_slav_gear['Material Cost Per Unit'] = df_utility.loc['Slave Gearbox','Material Bare Cost']*hist_cost_index*size_factor
    df_slav_gear['Equipment Cost Per Unit'] = equipment_bare_cost('Slave Gearbox', df_state, df_utility, state)*hist_cost_index
    return df_slav_gear

def motor_controller_equip(df_utility, df_inputs,hist_cost_index, df_state, state, num_modules, size_factor):
    ...
    df_mc_equip['Material Cost Per Unit'] = df_utility.loc['Motor & Controller Equipment','Material Bare Cost']*hist_cost_index*size_factor
    df_mc_equip['Equipment Cost Per Unit'] = equipment_bare_cost('Motor & Controller Equipment', df_state, df_utility, state)*hist_cost_index
    return df_mc_equip


############ ELECTRICAL ############

def conduit_wiring(df_structural_bos, df_utility, hist_cost_index, df_state, state, size_factor):
    ...
    df_cond_wire['Material Cost Per Unit'] = material_bare_cost('Conduit, Wiring', df_state, df_utility, state)*hist_cost_index*size_factor
    df_cond_wire['Equipment Cost Per Unit'] = equipment_bare_cost('Conduit, Wiring', df_state, df_utility, state)*hist_cost_index
    
    return df_cond_wire

def grounding_cable(df_structural_bos, df_bom, df_utility, hist_cost_index, df_state, state, size_factor):
    ...
    df_ground['Material Cost Per Unit'] = material_bare_cost('Grounding, DC Cable', df_state, df_utility, state)*hist_cost_index*size_factor
    df_ground['Equipment Cost Per Unit'] = equipment_bare_cost('Grounding, DC Cable', df_state, df_utility, state)*hist_cost_index
    
    return df_ground

def junction_box(df_inv_calc, df_inputs, df_utility, df_state, state, hist_cost_index, size_factor, num_modules):
    ...
    df_junc['Material Cost Per Unit'] = material_bare_cost('Junction/Combiner Boxes', df_state, df_utility, state)*hist_cost_index*size_factor
    df_junc['Equipment Cost Per Unit'] = equipment_bare_cost('Junction/Combiner Boxes', df_state, df_utility, state)*hist_cost_index
    
    return df_junc

def inverter_house(df_inv_calc, df_inputs, df_utility, df_state, state, hist_cost_index, project_size):
    ...
    df_inv_house['Material Cost Per Unit'] = material_bare_cost('Inverter House', df_state, df_utility, state)*hist_cost_index
    df_inv_house['Equipment Cost Per Unit'] = equipment_bare_cost('Inverter House', df_state, df_utility, state)*hist_cost_index
    
    return df_inv_house

def onsite_transmission(df_inv_house, df_utility, df_state, state, hist_cost_index):
    ...
    df_onsite_trans['Material Cost Per Unit'] = material_bare_cost('On-site Transmission', df_state, df_utility, state)*hist_cost_index
    df_onsite_trans['Equipment Cost Per Unit'] = equipment_bare_cost('On-site Transmission', df_state, df_utility, state)*hist_cost_index
    
    return df_onsite_trans

def pv_combining_switchgear(df_inputs, df_utility, df_state, state, hist_cost_index):
    ...
    df_pvcs['Material Cost Per Unit'] = material_bare_cost('PV Combining Switchgear (PVCS)', df_state, df_utility, state)*hist_cost_index
    df_pvcs['Equipment Cost Per Unit'] = equipment_bare_cost('PV Combining Switchgear (PVCS)', df_state, df_utility, state)*hist_cost_index
    
    return df_pvcs

def onsite_transformer_substation(df_inputs, df_utility, df_state, state, hist_cost_index, df_inv_calc, project_size, size_factor):
...
#     substation_cost_factor = stat.mean([df_inputs.loc['Substation Cost', A], df_inputs.loc['Substation Cost', B], df_inputs.loc['Substation Cost', C]])
    ...
    df_onsite_subs['Material Cost Per Unit'] = ((1240674.0-247620.0)/19.0+2540000.0/27.0)/2.0*project_size*size_factor*hist_cost_index* \
                                        substation_cost_factor* \
                                        float(df_state.loc[state, ('Location Factor', 'Material')])/100
    
    df_onsite_subs['Equipment Cost Per Unit'] = equipment_bare_cost('On-site transformer & Substation', df_state, df_utility, state)*hist_cost_index
    
    return df_onsite_subs

def site_prep_transmission(df_inputs, df_utility, df_state, state, hist_cost_index, project_size, poi, df_trans):
    ...
    if poi<trans_min:
        ...
        transmission_len = (poi-trans_min)*(df_trans.loc[trans_max,'Mile']-df_trans.loc[trans_min,'Mile'])/(trans_max-trans_min)+df_trans.loc[trans_min,'Mile']
...
#     transmission_line_factor = stat.mean([df_inputs.loc['Transmission line distance (mode=0.5 mile)', A], df_inputs.loc['Transmission line distance (mode=0.5 mile)', B], df_inputs.loc['Transmission line distance (mode=0.5 mile)', C]])
    ...
    df_site_prep_trans['Material Cost Per Unit'] = material_bare_cost('Site Preparation (Clearing and Grubbing) Transmission', df_state, df_utility, state)*hist_cost_index
    df_site_prep_trans['Equipment Cost Per Unit'] = equipment_bare_cost('Site Preparation (Clearing and Grubbing) Transmission', df_state, df_utility, state)*hist_cost_index
...
def conductor_cable_transmission(transmission_len, df_inputs, df_utility, transmission_line_factor, df_state, state, hist_cost_index):
    ...
    df_cond_trans['Material Cost Per Unit'] = material_bare_cost('Conductor and Cable Transmission', df_state, df_utility, state)*hist_cost_index
    df_cond_trans['Equipment Cost Per Unit'] = equipment_bare_cost('Conductor and Cable Transmission', df_state, df_utility, state)*hist_cost_index
...
def site_prep_distribution(transmission_len, df_inputs, df_utility, transmission_line_factor, df_state, state, hist_cost_index):
    ...
    df_site_prep_dist['Material Cost Per Unit'] = material_bare_cost('Site Preparation (Clearing and Grubbing) Distribution', df_state, df_utility, state)*hist_cost_index
    df_site_prep_dist['Equipment Cost Per Unit'] = equipment_bare_cost('Site Preparation (Clearing and Grubbing) Distribution', df_state, df_utility, state)*hist_cost_index
...
def conductor_cable_distribution(transmission_len, df_inputs, df_utility, transmission_line_factor, df_state, state, hist_cost_index):
    ...
    df_cond_dist['Material Cost Per Unit'] = material_bare_cost('Conductor and Cable Distribution', df_state, df_utility, state)*hist_cost_index
    df_cond_dist['Equipment Cost Per Unit'] = equipment_bare_cost('Conductor and Cable Distribution', df_state, df_utility, state)*hist_cost_index
...
def interconnection_fee(df_inputs, df_utility, df_state, state, other_inflation, project_size, poi):
    ...
    df_intercon_fee['Material Cost Per Unit'] = material_bare_cost('Interconnection Fee Per MW', df_state, df_utility, state)*other_inflation*poi*df_inputs.loc['Interconnection Fee Factor', D] \
    ...
    df_intercon_fee['Equipment Cost Per Unit'] = equipment_bare_cost('Interconnection Fee Per MW', df_state, df_utility, state)*other_inflation
