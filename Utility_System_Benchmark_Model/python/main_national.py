...
#calculates inflation based on 2021 cost data used
def get_inflation(df_cpi, year):
	df_c = df_cpi
	df_c['Change'] = df_c['cpi']/df_c.loc[2022,'cpi']
...
def get_other_inflation(df_cpi, year):
	df_c = df_cpi
	other_inflation = df_c.loc[year,'cpi']/df_c.loc[2020,'cpi']
	#print("Other Inflation", other_inflation)
	return other_inflation
...
other_inflation = get_other_inflation(df_cpi, year)
hist_cost_index = get_hist_cost_index(year,df_hist_cost)
labor_inflation = get_labor_inflation(year, labor_data_year, df_cpi)

spi_lastyr = df_steel_price.loc[year,'spi']
steel_inflation = spi_lastyr/df_steel_price.loc[2012,'spi']

spi_3yravg = (df_steel_price.loc[year,'spi']+df_steel_price.loc[year-1,'spi']+df_steel_price.loc[year-2,'spi'])/3
# steel_inflation = spi_3yravg/df_steel_price.loc[2012,'spi']
# steel_inflation_3yravg = spi_3yravg/df_steel_price.loc[2012,'spi']

#print("2012 SPI", df_steel_price.loc[2012,'spi'])
#print("Current Year SPI", spi_lastyr)
#print("Steel inflation - last year", steel_inflation)
# print("3 Year Avg SPI", spi_3yravg)
# print("Steel inflation - 3 year avg", steel_inflation_3yravg)
...
def final_table(project_size, poi, state, df_inv_calc, num_modules):
	...
	df_final.loc['Permitting Fee (if any)', state] = ft.get_permitting_cost(state, df_state, df_inputs, project_size, other_inflation)

	#Get Interconnection Fee Cost into final table
	df_final.loc['Interconnection Fee', state] = ft.get_interconnection_cost(df_utility, df_state, state, other_inflation, project_size, poi, df_inputs)
...
for tracker in tracker_list:
	...
	for project_size in project_size_list:
		...
		df_state_results[axis+" "+str(project_size)+" MWdc"+" "+str(poi)+" MWac"] = df_all_states
