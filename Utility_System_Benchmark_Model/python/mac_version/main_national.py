...
def final_table(project_size, state, df_inv_calc, num_modules):
	...
	df_final.loc['Epc Overhead',state], df_total_installed_cost, df_bare_cost, trans_dist_list = ft.get_oh_p_cost(df_inputs, df_install, df_utility, df_state, state, inflation, project_size, df_labor_weight, df_wage, df_epc, path)
...
export_path = '/'.join(os.path.dirname(os.path.abspath(__file__)).split('/')[0:-1]) + "/results/Q1 "+str(year)+" Utility Benchmark Table for Chart.xlsx"
