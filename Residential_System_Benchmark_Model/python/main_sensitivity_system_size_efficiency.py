...
if os.name=="nt":
	...
	export_path = '\\'.join(os.path.dirname(os.path.abspath(__file__)).split('\\')[0:-1]) + "\\results\\"+pre_loop_variables['Fiscal Year']+" Residential System Size Sensitivity.xlsx"
...
def main(inputs_dict, pre_loop_variables, states=benchmark_states, power=ifunc.power, export_path=export_path):
	...
	sensitivity_dict = {
		'Module Efficiency': {
			...
			'column': 'Value A',
			# 'sensitivity_value_list': [0.1,0.11,0.12,0.13,0.14,0.15,0.16,0.17,0.18,0.19,0.199,0.2,0.21,0.22,0.23,0.24,0.25,0.26,0.27,0.28,0.29,0.3,0.31,0.32,0.33,0.34,0.35,0.36,0.37,0.38,0.39,0.4],
			'sensitivity_value_list': [0.199],
		},
		# 'desired power': {
		# 	'location': inputs_dict['Main Inputs'],
		# 	'column': 'Value A',
		# 	'sensitivity_value_list': [2000,3000,4000,5000,6000,7000,8000,9000,10000,11000,12000],
		# },
		#
		# 'United States C': {
	...
	for variable in sensitivity_dict.keys():
		...
		for sensitivity_value in sensitivity_dict[variable]['sensitivity_value_list']:
			...
			#
			# df_final_tables['United States'].to_excel(writer, sheet_name = 'Sensitivty')
			...
			# df_us_module_costs.to_excel(writer, sheet_name = 'Module Related Costs')
				...
				# print(loc_variable, ';',(sensitivity_value), ';', print_roofspace, ';', module_length, ';', module_width, ';' , print_module_count, ';', print_active_area, ';', constraint, ';', dc_ac_ratio_string, ';', print_power, ';', state, ';', total_cost)
			print(system_size_dict['System Size'])
			print(df_final_table[('Mixed','Weighted Average')])
	...
	return df_final_tables['United States']
...
results = main(inputs_dict,pre_loop_variables)
writer = pd.ExcelWriter(export_path, engine = 'xlsxwriter')
results.to_excel(writer, sheet_name = 'Sensitivty')
